...
def main() -> None:
    ...
    df = prepare_frame(df_raw, product=args.product, grade=args.grade)

    n = len(df)
    if n < args.min_rows:
        log.error("Only %d heats after filtering (need ≥ %d). Exiting.", n, args.min_rows)
        sys.exit(1)

    scope_label = (
        f"Product {args.product}" if args.product
        else f"Grade {args.grade}" if args.grade
        else "All Heats"
    )
    ...
    slug = (args.product or args.grade or "all").lower().replace("-", "")
